...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const openMobileMenu = () => { mobileMenu.classList.add('open'); burgerBtn.setAttribute('aria-expanded', 'true'); document.body.style.overflow = 'hidden'; };
  const closeMobileMenu = () => { mobileMenu.classList.remove('open'); burgerBtn.setAttribute('aria-expanded', 'false'); document.body.style.overflow = ''; };
  if (burgerBtn && mobileMenu) {
    burgerBtn.addEventListener('click', () => { mobileMenu.classList.contains('open') ? closeMobileMenu() : openMobileMenu(); });
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => { if (entry.isIntersecting) entry.target.classList.add('shown'); });
  }, { threshold: 0.15 });
  ...
  principles.forEach((p) => {
    p.addEventListener('mouseenter', () => { principles.forEach((o) => o.classList.toggle('dimmed', o !== p)); });
    p.addEventListener('mouseleave', () => { principles.forEach((o) => o.classList.remove('dimmed')); });
  });
  ...
  const setOpenFaq = (item) => {
    faqItems.forEach((i) => {
      ...
      i.querySelector('.faq-question').setAttribute('aria-expanded', String(isOpen));
    });
  };
  const closeAllFaq = () => {
    faqItems.forEach((i) => { i.classList.remove('open'); i.querySelector('.faq-question').setAttribute('aria-expanded', 'false'); });
  };
